...
def sql_parse(sql):  #insert delete update select
    ...
    sql_l=sql.split(' ')   #根据空格切分
    func=sql_l[0]      #按照功能分发到不同的函数去解析
...
def select_parse(sql_l):
    ...
    # print('from in the select_parse [%s]' %sql_l)
    sql_dic={
        'func':select,
        'select':[],  #查询字段
        'from':[], #数据.表
...
#处理多个where的情况
def where_parse(where_l):#['id>','4', 'and', 'id','<10']
    res = []
    key = ['and', 'or', 'not']
    char = ''
    for i in where_l:
...
def select(sql_dic):
    ...
    # first：from
    db, table = sql_dic.get('from')[0].split('.')  #获取数据库名和表名
